...
\* \*\*Hazard Detection Unit:\*\* Automatically identifies immediate Load-Use hazards, dynamically dropping `PCWrite` and `IF\_ID\_Write` control lines to freeze preceding instructions while injecting an execution "bubble" (NOP) to preserve state stability.
